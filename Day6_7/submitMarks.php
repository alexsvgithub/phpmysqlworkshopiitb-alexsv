...
if(!empty($_POST))
{
	...
	$s3=$_POST['s3'];
	$roll=$_POST['roll'];
	if($s1==NULL || $s2==NULL || $s3==NULL ||  $roll==NULL)
	{
		//
	}
	else
	{
		...
		if($per<=35)
		{
			...
			elseif($per>=51 && $per<=60)
				...
				$sql=mysqli_query($al,"INSERT INTO marks(roll,sem,s1,s2,s3,total,percent,result) VALUES('$roll','$sem','$s1','$s2','$s3','$total','$per','$result')");
				if($sql)
				{
...
<div align="center">
<span class="head">Student Result </span>
<hr class="hr" />
<br />
<br />
